...
def refresh_user_data():
    """Reset a user's swipes and matches while preserving profile information"""
    try:
        ...
        return jsonify({'message': 'Swipes and matches have been refreshed successfully. Your profile information remains intact.'}), 200
    except Exception as e:
...
def refresh_user():
    """Reset the current user's swipes and matches"""
    return refresh_user_data()

def clear_all_data():
    """Clear all data from the database and recreate tables"""
    try:
        # Delete all records from all tables
        db.session.query(Match).delete()
        db.session.query(UserSwipe).delete()
        db.session.query(Experience).delete()
        db.session.query(User).delete()
        db.session.commit()
        print("All existing data has been cleared from the database")
        return True
    except Exception as e:
...
# Function kept for compatibility, but now empty
def create_demo_matches():
    """Previously created demo matches, now empty for fresh starts"""
    # This function is now empty as we're removing all demo data
    pass
...
with app.app_context():
    try:
        ...
        db.create_all()  # This only creates tables that don't exist, won't delete data
...
def cas_callback():
    ...
    try:
        ...
        # If user doesn't exist, create a new user with Princeton credentials
        if not user:
            # Extract first and last name from netid if possible
            name_parts = netid.split('.')
            if len(name_parts) > 1:
                first_name = name_parts[0].capitalize()
                last_name = name_parts[1].capitalize()
                full_name = f"{first_name} {last_name}"
            else:
                full_name = f"{netid.capitalize()}"
            
            # Create a new user with the netid and Princeton info
            new_user = User(
                username=netid,
                name=full_name,
                gender='Other',  # Default, user can update later
                class_year=2025,  # Default class year
                interests='{"hiking": true, "dining": true, "movies": true, "study": true}',  # Default interests
...
def get_current_user():
    ...
    try:
        ...
        if 'user_info' in session:
            ...
            # Find the user with the matching netid
            user = User.query.filter_by(username=netid).first()
